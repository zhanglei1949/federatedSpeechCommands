...
from mixup import *
from federated_utils import getLenOfGradientVector, transListOfArraysToArrays, getShapeList,transNumpyGrad2Cuda
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        for batch in pbar:
            ...
            if (current_client == 0):
                ...
                A = np.random.randint(0, 10000,size = (args.matrix_size, args.matrix_size)) 
                ...
                C = np.random.randint(0, 10000, size = (2 * args.matrix_size, 3*args.matrix_size))
                for i in range(0, args.matrix_size):
                ...
                sigma[:min(C.shape[0],C.shape[1]), :min(C.shape[0],C.shape[1])] = np.diag(s)
                assert(np.allclose(C, np.dot(u, np.dot(sigma, vh))) == True)
                # linear independent group(null space)
                ns = scipy.linalg.null_space(C) # (3000, 1000) we use the first args.matrix_size
                print('ns', ns.shape)
            ...
            for i in range(args.matrix_size):
                r_new += r[i] * ns[:,i:i+1]
            num_paddings = math.ceil(float(flatterned_grad.shape[0])/args.matrix_size) * args.matrix_size  - flatterned_grad.shape[0]
            print('flatterned', flatterned_grad.shape, 'need padding', num_paddings) #np.array
            # extent to 2n
            flatterned_grad_extended = np.zeros((flatterned_grad.shape[0] + num_paddings))
            flatterned_grad_extended[:flatterned_grad.shape[0]] = flatterned_grad
            #print(flatterned_grad_extended[:20])
            current_client_grad_after_random = np.zeros((3*flatterned_grad_extended.shape[0]))
            new_grad = np.random.randint(0, 10000, size = (3 * args.matrix_size, 1))
            for i in range(0, flatterned_grad_extended.shape[0], args.matrix_size):
                ...
                current_client_grad_after_random[3*i:3*i + 3*args.matrix_size] = np.squeeze(randomized_gradient)
            ...
            if (current_client == args.clients - 1):
                ...
                for i in range(0, global_grad_sum.shape[0] , 3*args.matrix_size):
                    tmp = np.dot(u, np.dot(sigma, global_grad_sum[i : i + 3*args.matrix_size]))
                    for j in range(args.matrix_size): 
                        alpha[j] = tmp[S_j[j]]
                    res[int(i/3) : int(i/3) + args.matrix_size] = (np.dot(A_inv, alpha)).squeeze()
